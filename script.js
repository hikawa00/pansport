...
// 存储赛程数据
const events = {
    "2025-01-01": { sport: '澳网', time: '11:30', description: '对手A vs 对手B,地点A', predicted: null, interestLevel: null },
    "2025-01-02": { sport: 'F1', time: '14:00', description: '赛道A', predicted: null, interestLevel: null },
    "2025-01-03": { sport: '英超', time: '23:00', description: '球队A vs 球队B,地点B', predicted: null, interestLevel: null }
};
...
tabButtons.forEach(button => {
    button.addEventListener('click', () => {
        ...
        if (currentSportType === 'football') {
            ...
            // F1和网球显示空状态
            showEmptyState(currentSportType);
        }
    });
});

// 显示空状态的函数
function showEmptyState(sportType) {
    const matchListElement = document.getElementById('upcoming-list');
    matchListElement.innerHTML = `
        <div class="empty-state">
            <p>${sportType === 'f1' ? 'F1' : '网球'}赛事数据正在准备中...</p>
        </div>
    `;
}
